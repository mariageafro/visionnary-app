 ...
 */
export function reorderOnDrop<T extends { id: string; order: number }>(list: T[], draggedId: string, targetId: string, outside?: T): { orders: Map<string, number>; crossed: boolean } | null {
  ...
  const at = rest.findIndex((item) => item.id === targetId) + (from >= 0 && from < targetIndex ? 1 : 0);
...
interface Handlers {
  /** Dépôt sur une autre vignette (attribut data-reorder="id"). */
  onDropTile: (draggedId: string, targetId: string) => void;
...
export function usePointerReorder(handlers: Handlers) {
  ...
  return useCallback((event: ReactPointerEvent, id: string) => {
    ...
    const setHover = (next: HTMLElement | null) => {
      ...
      hover?.classList.remove("is-over");
      hover = next;
    ...
    const move = (e: PointerEvent) => {
      ...
      const under = document
        ...
        .find((el): el is HTMLElement => el instanceof HTMLElement && ((el.dataset.reorder !== undefined && el.dataset.reorder !== id) || el.dataset.reorderSection !== undefined));
      setHover(under ?? null);
    };
    const end = (cancelled: boolean) => {
      ...
      if (target.dataset.reorder !== undefined) latest.current.onDropTile(id, target.dataset.reorder);
