            ...
            >
              ...
              {!sectionConfig?.collapsed && <div className="insp-grid">
                {list.map((i) => (
                  ...
                  {(() => { const series = seriesMedia(media, i); const taken = series.filter((m) => doneAngles(i).has(m.id)).length; return series.length > 1 ? <span className={"pose-angles" + (taken === series.length ? " is-complete" : "")} title={`${series.length} angles · ${taken} pris`}><Layers size={16} /> <b>{series.length}</b> angles{taken ? ` · ${taken} pris` : ""}</span> : null; })()}
