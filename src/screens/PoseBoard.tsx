              ...
              {list.map((pose, n) => {
                ...
                return (
                  <div key={pose.id} className={"pose-tile" + (done(pose) ? " is-done" : "") + (pose.favorite === true ? " is-fav" : "")} data-reorder={pose.id} data-merge="">
                    ...
                    {series.length > 1 && <span className={"pose-angles" + (taken === series.length ? " is-complete" : "")} title={`${series.length} angles · ${taken} pris`}><Layers size={16} /> <b>{series.length}</b> angles{taken ? ` · ${taken} pris` : ""}</span>}
